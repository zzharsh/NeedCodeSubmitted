...
public:
    ...
    vector<vector<int>> threeSum(vector<int>& nums) {
        ...
        {
            remaining_values.erase(remaining_values.find(it1->first));
            unordered_multimap<int, int>  jRemaining_values = remaining_values;
            if(jRemaining_values.size()>0)
            {
                for (auto it = jRemaining_values.begin(); it!=jRemaining_values.end(); ++it)
                    cout<<it->first<<" ";
            cout<<"\n";
            }
            for(auto it2 = next(it1); it2!=sorted_values.end(); ++it2)
            {
                jRemaining_values.erase(jRemaining_values.find(it2->first));
