class Solution {
public:
    std::vector<std::vector<char>> Lists = vector<vector<char>>(26, vector<char>());
    ...
    string DFS(char ch, unordered_set<char> current)
    {
        ...
        current.insert(ch);
        string str = "";
        for(int i = 0 ; i < Lists[ch].size(); ++i)
        {
            str = DFS(Lists[ch][i], current);
        }
    ...
    string foreignDictionary(vector<string>& words) {
        ...
        for(int i = 0; i < n-1; ++i)
        {
            ...
            if (j < words[i].size() && j < words[i+1].size())
            {
                exists[words[i][j] - 'a'] = true;
                exists[words[i+1][j] - 'a'] = true;
                Lists[words[i+1][j] - 'a'].push_back(words[i][j] - 'a');
            }
